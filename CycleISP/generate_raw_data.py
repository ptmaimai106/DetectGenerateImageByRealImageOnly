...
with torch.no_grad():
    for ii, data in enumerate(tqdm(test_loader), 0):
        ...
        for j in range(raw_gt.shape[0]):
            ...
            # maichanged
            clean_unpacked_ubyte = img_as_ubyte(clean_unpacked)
            img = Image.fromarray(clean_unpacked_ubyte)
            os.makedirs(args.result_dir + 'png/clean/', exist_ok=True)
            img.save(args.result_dir + 'png/clean/' + filename[:-4] + '.png')
            ...
            # maichanged
            noisy_unpacked_ubyte = img_as_ubyte(noisy_unpacked)
            img = Image.fromarray(noisy_unpacked_ubyte)
            os.makedirs(args.result_dir + 'png/noisy/', exist_ok=True)
            img.save(os.path.join(args.result_dir, 'png/noisy/', filename[:-4] + '.png'))
