...
with torch.no_grad():
    for ii, data in enumerate(tqdm(test_loader), 0):
        ...
        for j in range(raw_gt.shape[0]):   ## Use loop to add different noise to different images.
            ...
            os.makedirs(args.result_dir + 'clean/', exist_ok=True)
            os.makedirs(args.result_dir + 'noisy/', exist_ok=True)

            rgb_clean_ubyte = img_as_ubyte(rgb_clean)
            img_clean = Image.fromarray(rgb_clean_ubyte)
            img_clean.save(os.path.join(args.result_dir, 'clean', filename[:-4] + '.png'))

            rgb_noisy_ubyte = img_as_ubyte(rgb_noisy)
            img_noisy = Image.fromarray(rgb_noisy_ubyte)
            img_noisy.save(os.path.join(args.result_dir, 'noisy', filename[:-4] + '.png'))
